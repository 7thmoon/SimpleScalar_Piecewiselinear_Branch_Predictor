...
typedef struct {
	...
	int *ghr_entry;  /* entry of Global history branch register */
	int *shr_entry;  /* entry of Speculative history branch register */
	md_addr_t *ga_entry; /* array of Global address */
	md_addr_t *sa_entry; /* array of Speculative address */
...
#define THETA(prcpt_history)		((int) (2.14 * (prcpt_history + 1) + 20.58))
...
bpred_create(enum bpred_class class,	/* type of predictor to create */
		 ...
		 unsigned int weight_size, /* weight bit width */
	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredComb:
    ...
    /* 2-level component */
    pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, l1size, l2size, shift_width, 0, xor);
  ...
  case BPred2Level:
    pred->dirpred.twolev = 
      bpred_dir_create(class, l1size, l2size, shift_width, 0, xor);

    break;
    
  case BPredPerceptron:
    pred->dirpred.bimod =
      bpred_dir_create(class, bimod_size, l1size, shift_width, weight_size, 0);   
...
bpred_dir_create (
  ...
  unsigned int shift_width,	/* history register length */
  unsigned int weight_size,	/* weight bits width */
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPredPerceptron:
	...
	pred_dir->config.neural.num_prcpt = 1024;
	pred_dir->config.neural.weight_bits = weight_size;
	pred_dir->config.neural.prcpt_history = shift_width; 
	pred_dir->config.neural.n = l1size;
	pred_dir->config.neural.m = l2size;
	pred_dir->config.neural.h = shift_width; /* h is the same as prcpt_history */
	int n = pred_dir->config.neural.n;
	int m = pred_dir->config.neural.m;
	int h = pred_dir->config.neural.h;

    if (!(pred_dir->config.neural.weights_table =
		calloc(n*m, sizeof(int)*(h+1))))
	  //calloc(l1size, sizeof(int)*(shift_width+1))))
	  fatal("cannot allocate perceptrons storage");
	  
	if (!(pred_dir->config.neural.ghr =
		calloc(h, sizeof(int))))
	fatal("cannot allocate global history register storage");
	
	if (!(pred_dir->config.neural.shr =
		calloc(h, sizeof(int))))
	fatal("cannot allocate speculative history register storage");
	
	if (!(pred_dir->config.neural.ga =
		calloc(h, sizeof(unsigned long long))))
	fatal("cannot allocate global address storage");
	
	if (!(pred_dir->config.neural.sa =
		calloc(h, sizeof(unsigned long long))))
	fatal("cannot allocate global address storage");
    break;
    ...
    case BPredPerceptron:
		{
			...
			***************************************************************/
			int i, index_n, index_m, output, n, m, h;
			n = pred_dir->config.neural.n;
			m = pred_dir->config.neural.m;
			h = pred_dir->config.neural.h;
			prcpt_update_t* pred_state;
			int* w;
			int* weight_entry;

			
			index_n = NEURAL_HASH(n, baddr);
			w = &pred_dir->config.neural.weights_table[m*(h+1)*index_n];
			output = *w;

			for (i=0; i< h; i++) 
			{
				if (pred_dir->config.neural.sa[i] != 0 ){
				index_m = NEURAL_HASH(m, pred_dir->config.neural.sa[i]);
				w = &pred_dir->config.neural.weights_table[m*(h+1)*index_n + (h+1)*index_m + i+1];
				if (pred_dir->config.neural.shr[i] == 1)
					output += *w;
				else
					output += -*w;
				}
			...
			for (i=h-1; i >0; i--) 
			{	
				pred_dir->config.neural.sa[i]= pred_dir->config.neural.sa[i-1];
				pred_dir->config.neural.shr[i] = pred_dir->config.neural.shr[i-1];
			}
			
			pred_dir->config.neural.sa[0] = baddr;	
			pred_dir->config.neural.shr[0] = output >= 0;
			weight_entry = &pred_dir->config.neural.weights_table[m*(h+1)*index_n];

	
			if (!(pred_state = calloc(1, sizeof(prcpt_update_t))))
				fatal("cannot allocate perceptron_update storage");
			pred_state->output = output;
			pred_state->weights_table_entry = weight_entry;
			pred_state->shr_entry = &pred_dir->config.neural.shr[0];
			pred_state->sa_entry = &pred_dir->config.neural.sa[0];;
    ...
    {	
	  if(pred->class == BPredPerceptron)
	  {
		...
		*************************************************************/
		...
		int i;
		int *w;
		int m = pred->dirpred.bimod->config.neural.m;
		int h = pred->dirpred.bimod->config.neural.h;
		int index_m;
		
		for (i=h-1; i >0; i--) 
		{	
			pred->dirpred.bimod->config.neural.ghr[i] = pred->dirpred.bimod->config.neural.ghr[i-1];
			pred->dirpred.bimod->config.neural.ga[i] = pred->dirpred.bimod->config.neural.ga[i-1];
		}
		pred->dirpred.bimod->config.neural.ghr[0] = taken;
		pred->dirpred.bimod->config.neural.ga[0] = baddr;
		if (u->prediction != taken)
		{
			for (i=h-1; i >=0; i--) 
			{	
				pred->dirpred.bimod->config.neural.shr[i] = pred->dirpred.bimod->config.neural.ghr[i];
				pred->dirpred.bimod->config.neural.sa[i] = pred->dirpred.bimod->config.neural.ga[i];
			}
		}
		
		if (u->output < THETA(pred->dirpred.bimod->config.neural.prcpt_history)
			&& u->output > -THETA(pred->dirpred.bimod->config.neural.prcpt_history)
			)
			needUpdate = 1;
		if (u->prediction != taken)
		{
		...
		if(needUpdate)
		{
			...
			for (i=0; i < h; i++) 
			{
				if (u->sa_entry[i] != 0)
				{
					index_m = NEURAL_HASH(m, u->sa_entry[i]);
					w = &u->weights_table_entry[0] + (h+1)*index_m + i;
					if (pred->dirpred.bimod->config.neural.shr[i] == taken)
						(*w)++;
					else
						(*w)--;
					if (*w > MAX_WEIGHT(pred->dirpred.bimod->config.neural.weight_bits)) *w = 
