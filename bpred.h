...
struct bpred_dir_t {
  ...
  union {
	...
	struct {
	  ...
	  int shift_width;		/* length of global history regs */
	  int n;
	  int m;
	  int h;
	  int *weights_table;		/* every entry is an array of weights, a perceptron */
	  md_addr_t *ga;			/* every entry is an array of Global address */
	  md_addr_t *sa;			/* every entry is an array of Speculative address */
	  int *ghr;			/* every entry is an outcome of Global history branch */
	  int *shr;			/* every entry is an outcome of Speculative history branch */
